...
const urlChecks = {
  ...
  SCOPE_CHECK: (
    ...
    logInfo(
      "Portal_API_WHITELIST_SCOPE_CHECK : Middleware for URL [ " +
        REQ_URL +
        " ]"
    );
    const orgData = _.get(req, "session.orgs")
      ? _.get(req, "session.orgs")
      : [];
    const orgId = _.get(req, "query.orgId") ? _.get(req, "query.orgId") : "";
    if (_.isEmpty(orgId) || _.isEmpty(orgData)) {
      return reject(
        "User doesn't have appropriate organisation for scope check 1"
      );
    }
    // tslint:disable-next-line: no-any
    let scopeRoles: any = [];
    ...
    if (_.isEmpty(scopeRoles)) {
      return reject(
        "User doesn't have appropriate organisation for scope check"
      );
    }
...
const respond419 = (req: Request, res: Response) => {
  ...
  if (_.includes(REQ_URL, "/reset")) {
    ...
    const err = {
      msg: "API WHITELIST :: Unauthorized access for API [ " + REQ_URL + " ]",
      url: REQ_URL,
    };
    logError(err.msg);
    res.status(419);
...
export function apiWhiteListLogger() {
  return (req: Request, res: Response, next: NextFunction) => {
    if (
      req.path === "/" ||
      checkIsStaticRoute(req.path) ||
      _.includes(req.path, "public")
    ) {
      next();
      return;
    }
    ...
    if (!_.includes(req.path, "/resource") && req.session) {
      if (
        !("userRoles" in req.session) ||
        ("userRoles" in req.session && req.session.userRoles.length === 0)
      ) {
        logError(
          "Portal_API_WHITELIST_LOGGER: User needs to authenticated themselves"
        );
        respond419(req, res);
      } else {
